...
export const EvvmInfo = () => {
  const { loading, evvmID, stakingAddress, nameserviceAddress } = useEvvm();
  ...
  return (
    <div className="bg-secondary rounded-xl p-3">
      ...
      <p>
        Staking: {loading ? <Spinner className="inline" /> : stakingAddress}
      </p>
      <p>
        Name Service:{" "}
        {loading ? <Spinner className="inline" /> : nameserviceAddress}
      </p>
    </div>
  );
};
